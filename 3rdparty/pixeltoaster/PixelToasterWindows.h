...
namespace PixelToaster
{
	...
	public:
		...
		void capture_mouse()
		{
			...
			GetClipCursor(&old_clip_cursor);
			ClipCursor(&rect);
		}

		void uncapture_mouse()
		{
			ClipCursor(&old_clip_cursor);
		}
		...
		LRESULT CALLBACK WindowProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
		{
			switch ( uMsg )
			{
				...
				case WM_MOUSEWHEEL:
				{
					...
					mouse.x = (float) GET_X_LPARAM( lParam ) * widthRatio;
					mouse.y = (float) GET_Y_LPARAM( lParam ) * heightRatio;
					short wheel_amount = HIWORD(wParam);
					if ( _listener )
						_listener->onMouseWheel( display->wrapper() ? *display->wrapper() : *display, mouse, wheel_amount );
				}
				break;
						...
						if ( mode_width >= width && mode_height >= height && ( bestWidth == 0 || mode_width <= bestWidth && mode_height <= bestHeight ) )
						{
							if ( modeset_enable ) {
								if ( mode_height == modeset_height && mode_width == modeset_width ) {
									bestWidth = mode_width;
									bestHeight = mode_height;
								}
							} else {
								bestWidth = mode_width;
								bestHeight = mode_height;
